...
#     global buffer
#     ser.write(signal.encode())  # Convert the signal to bytes and write it to the serial port
#     buffer += 1
#     if buffer >= 10:  # Send signal every 10 loops
#         buffer = 0
...
while time.time() - start_time <= 10:  # Run the loop for 10 seconds
    ...
    for encodeFace, faceLoc in zip(encodeCurrFrame, faceCurrFrame):
        ...
        if matches[matchIndex] and faceDis[matchIndex] < 0.5:
            ...
            # bbox = 60 + x1, 240 + y1, x2 - x1, y2 - y1
            color = (0, 255, 0)
            # imgBackground = cvzone.cornerRect(imgBackground, bbox,colorC=(0, 255, 0),colorR=(0, 255, 0), rt=0, t=3)
            ...
            if(time.time()- start_time>5):
                # send_signal(signal)
                print("Sending recognized signal to the arduino!")
                cap.release()
                cv2.destroyAllWindows()
                # ser.close()
                exit()
        else:
            #print(matches[matchIndex], faceDis[matchIndex])
            # Face not authorized
            #print("Sending unrecognized signal to the arduino!")
            color = (0,0,255)            
            y1, x2, y2, x1 = faceLoc
            y1, x2, y2, x1 = y1 * 4, x2 * 4, y2 * 4, x1 * 4
...
cap.release()
cv2.destroyAllWindows()
print("Unrecognized Signal is sent to the Arduino!")
# send_signal('0')  # Send '0' to the Arduino after 10 seconds
# ser.close()